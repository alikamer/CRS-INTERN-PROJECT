...
const AdminQueue = () => {
  ...
  return (
    <div className="space-y-6">
      {/* Page Header */}
      <div>
        <h1 className="text-2xl font-bold text-[#1F1F1F] tracking-tight">Onay Bekleyen Fiş Kuyruğu</h1>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <div className="lg:col-span-1 ga4-card p-5 space-y-4">
          <h3 className="font-bold text-[#1F1F1F] text-sm border-b border-[#E1E3E1] pb-3 flex items-center justify-between">
            ...
            <span className="ga4-badge-amber text-[12px] font-bold px-2 py-0.5 rounded-full">{pendingReceipts.length}</span>
          </h3>

          {loading ? (
            <p className="text-[#747775] text-sm py-4 text-center">Kuyruk yükleniyor...</p>
              ...
              {pendingReceipts.map((r) => (
                ...
                >
                  <div className="flex justify-between items-start">
                    <div>
                      <p className="font-bold text-sm text-[#1F1F1F]">{r.brandName}</p>
                      <p className="text-[13px] text-[#747775]">{new Date(r.receiptDate).toLocaleDateString('tr-TR')}</p>
                    </div>
                    <span className="font-bold text-sm text-[#0B57D0]">₺{r.totalAmount.toFixed(2)}</span>
                  </div>
                </div>
              ))}
            </div>
        ...
        <div className="lg:col-span-2 ga4-card p-6">
          {!selectedReceipt ? (
            <div className="py-20 text-center text-[#747775] space-y-3">
              ...
              <p className="text-sm font-medium">Detay ve onay için soldaki listeden bir fiş seçin.</p>
            </div>
          ) : (
            <div className="space-y-6">
              <div className="flex justify-between items-center border-b border-[#E1E3E1] pb-4">
                <div>
                  ...
                  <p className="text-[13px] text-[#747775]">Fiş ID: {selectedReceipt.id}</p>
                </div>
                <button
                  onClick={() => handleApprove(selectedReceipt.id)}
                  className="bg-[#137333] hover:bg-[#0F5C29] text-white px-4 py-2 rounded-full font-bold text-sm transition-all flex items-center gap-1.5"
                >
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                <div className="bg-[#F0F4F9] border border-[#E1E3E1] rounded-2xl p-4 flex flex-col items-center justify-center min-h-[220px]">
                  {selectedReceipt.imageUrl ? (
                    ...
                    <div className="text-center text-[#747775] text-sm">
                ...
                <div className="space-y-4">
                  <h4 className="font-bold text-[#1F1F1F] text-sm uppercase tracking-wider">
                    Ürün Kalemleri ({selectedReceipt.items?.length || 0})
                  </h4>

                  <div className="space-y-2 max-h-40 overflow-y-auto">
                    {selectedReceipt.items?.map((item) => (
                      <div key={item.id} className="flex justify-between items-center bg-[#F0F4F9] p-2.5 rounded-xl text-sm">
                        <div>
                  ...
                  <div className="border-t border-[#E1E3E1] pt-3 space-y-2.5">
                    <h5 className="text-[12px] font-bold text-[#747775] uppercase tracking-wider">Yeni Kalem Ekle</h5>
                    <div className="grid grid-cols-2 gap-2">
                      <input 
                        ...
                        className="px-3 py-1.5 border border-[#E1E3E1] rounded-xl text-sm bg-white outline-none focus:border-[#0B57D0]"
                      />
                      <input 
                        type="number" 
                    ...
                    <button 
                      ...
                      className="w-full bg-[#0B57D0] hover:bg-[#0842A0] text-white text-sm font-semibold py-2 rounded-full transition-colors flex items-center justify-center gap-1"
                    >
